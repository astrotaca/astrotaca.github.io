const guidesData = [
{
    ...
    fullContent: [
        ...
        { type: 'paragraph', content: 'The <strong>Advanced Sequencer</strong> uses three container types:' },
        { type: 'list', items: [
        ...
        { type: 'paragraph', content: 'Within containers, you add <strong>instructions</strong> (commands), <strong>triggers</strong> (event-driven actions), and <strong>loop conditions</strong> (criteria for when to continue or stop).' },
        { type: 'heading', content: 'Critical Components for Automation' },
        { type: 'heading', content: 'Triggers: Event-Driven Actions' },
        { type: 'paragraph', content: 'Triggers fire automatically when conditions are met. They evaluate after every instruction and execute without stopping the sequence.' },
        { type: 'list', items: [
        ...
        { type: 'heading', content: 'Loop Conditions: When to Continue' },
        { type: 'paragraph', content: 'Loop conditions determine whether a container repeats. Multiple conditions stack (all must remain true).' },
        { type: 'list', items: [
        ...
        { type: 'paragraph', content: 'Example: Loop Until Time (Nautical Dawn) AND Loop While Altitude Above Horizon.' },
        { type: 'heading', content: 'Filter Offsets' },
        { type: 'paragraph', content: 'Filter offsets let NINA adjust focus instantly when switching filters. Use the Filter Offset Calculator plugin to set this up once and save time.' },
        { type: 'heading', content: 'Target Sequence Structure' },
        { type: 'paragraph', content: 'Here\u2019s how I structure my LRGB target sequences:' },
        { type: 'list', items: [
        ...
        { type: 'image', src: '/guides/nina-sequence-sleep.jpg', alt: 'LRGB target sequence structure in NINA' },
        { type: 'heading', content: 'Key Design Decisions' },
        { type: 'list', items: [
        ...
        { type: 'heading', content: 'Startup & Shutdown' },
        { type: 'list', items: [
        ...
        { type: 'heading', content: 'Advanced: Target Scheduler Plugin' },
        { type: 'paragraph', content: 'For complex multi-target sessions, the Target Scheduler plugin can select optimal targets automatically.' },
        { type: 'heading', content: 'Safety for Unattended Operation' },
        { type: 'list', items: [
        ...
        { type: 'heading', content: 'Common Failures and Fixes' },
        { type: 'list', items: [
        ...
        { type: 'heading', content: 'Building Reliable Automation' },
        { type: 'paragraph', content: 'Start simple. Build one working target with basic triggers and loop conditions. Once solid, add complexity. Test failures (disconnect USB, cover guide scope, etc.) until your sequence recovers reliably.' }
    ]
},
{
    ...
    fullContent: [
        { type: 'paragraph', content: 'This guide covers building ASCOM drivers to integrate custom hardware with NINA and other imaging software. It focuses on practical implementation and what actually matters for reliability.' },
        { type: 'paragraph', content: '<strong>Prerequisites:</strong> You should understand serial communication (Arduino/ESP32) and basic programming. This guide focuses on what\'s involved in making a working driver.' },
        ...
        { type: 'paragraph', content: 'Each interface defines required methods and properties. Templates include these. You fill in the serial communication logic.' },
        { type: 'heading', content: 'Driver Architecture' },
        { type: 'paragraph', content: '<strong>Driver Class:</strong> Implements the ASCOM interface. Handles connection state for each client. Routes method calls to hardware class. Provides setup dialog for COM port selection.' },
        { type: 'paragraph', content: '<strong>Hardware Class:</strong> Manages the serial connection. Single shared instance for all clients. Tracks actual hardware state. Runs background threads for reading serial responses.' },
        { type: 'paragraph', content: 'This structure lets multiple programs connect to your driver while maintaining one hardware connection.' },
        { type: 'heading', content: 'Serial Communication Pattern' },
        { type: 'paragraph', content: 'When NINA calls OpenCover(), your driver sends "open" over serial. Hardware responds with status messages. Driver updates internal state and reports back to NINA.' },
        { type: 'paragraph', content: 'Commands like <code>open</code>, <code>close</code>, <code>led:50</code>. Responses like <code>OPENING</code>, <code>OPEN</code>, <code>CLOSED</code>.' },
        { type: 'paragraph', content: 'Run a background thread that continuously reads serial and updates state variables. Never block ASCOM method calls waiting for hardware responses. Methods must return quickly (under 1 second).' },
        { type: 'paragraph', content: 'NINA polls properties like CoverState or IsMoving to track completion. Your background thread updates these as hardware sends status messages.' },
        { type: 'heading', content: 'Problems I Hit Building Mine' },
        { type: 'paragraph', content: 'These weren\'t obvious from ASCOM documentation. Most broke during real imaging sessions, not bench testing.' },
        { type: 'paragraph', content: '<strong>USB Serial Reliability</strong><br>Connections drop randomly. Packets get lost. Implement retry logic for failed commands. Send critical status messages multiple times from hardware with short delays between each. If one packet drops, others get through.' },
        { type: 'paragraph', content: '<strong>Hardware Timing</strong><br>Servo takes 3 seconds to fully open. Without progress updates, NINA thinks the driver hung. Send status messages during operation so software knows it\'s executing, not stuck.' },
        { type: 'paragraph', content: '<strong>Initialization Delay</strong><br>Wait 1.5 seconds after opening serial port before sending commands. Microcontroller needs time to initialize. First command fails without this delay.' },
        ...
        { type: 'paragraph', content: 'Driver should recover from all failures without crashing or leaving hardware in bad state.' },
        { type: 'paragraph', content: '<strong>Logging</strong><br>Enable ASCOM TraceLogger. Log every command sent, response received, state change. When something breaks during unattended imaging, logs are your only diagnostic.' },
        ...
        { type: 'paragraph', content: 'Store COM port and device settings in ASCOM Profile. Provides persistent storage across sessions. Use the Profile object, don\'t write directly to registry.' },
        { type: 'paragraph', content: 'Setup dialog should have COM port dropdown with refresh button. If user opens setup while connected, disconnect temporarily, allow changes, reconnect after.' },
        { type: 'heading', content: 'Integration' },
        { type: 'paragraph', content: 'Register driver with ASCOM using regasm or an installer. Connect in NINA via Equipment menu. Test full imaging sequence. Run unattended overnight to verify reliability under real conditions.' }
    ]
},
{
    ...
    fullContent: [
        { type: 'paragraph', content: 'After stacking your astrophotography data into a FITS file, the result might look underwhelming: dim, flat, and possibly green. This is expected, and it\'s due to the image being in a linear state.' },
        { type: 'heading', content: 'Linear vs. Stretched Images' },
        { type: 'paragraph', content: 'Most processing software saves stacked images in linear format. This means:' },
        { type: 'list', items: [
        ...
        { type: 'paragraph', content: 'Linear images are optimized for data retention, not for display. They contain the full dynamic range from your exposures, which is important for scientific use or high-quality post-processing.' },
        { type: 'paragraph', content: 'On the other hand, a stretched image has had a nonlinear transformation applied (typically a histogram stretch or curve), which redistributes pixel brightness to make faint structures visible on a standard screen.' },
        { type: 'heading', content: 'Why It Looks Green' },
        { type: 'paragraph', content: 'If you\'re using a one-shot color (OSC) camera, your sensor has a Bayer matrix with 2\u00d7 as many green pixels as red or blue. After stacking, the green channel is usually dominant unless color calibration has been applied. Most stackers don\'t do this by default.' },
        { type: 'paragraph', content: 'This is corrected later in the processing pipeline through proper white balance or photometric color calibration.' },
        { type: 'heading', content: 'What to Check in the Stack' },
        { type: 'paragraph', content: 'Although the image will look dim or unprocessed, you can still evaluate a few technical aspects:' },
        { type: 'list', items: [
        ...
        { type: 'heading', content: 'When to Stretch' },
        { type: 'paragraph', content: 'You should only stretch the image after performing critical preprocessing steps like:' },
        { type: 'list', items: [
            'Background extraction',
            'Color calibration',
            'Noise reduction (if working in linear space)'
        ]},
        { type: 'paragraph', content: 'Once you\'re done with those, a non-linear stretch brings out detail and color, and makes the image suitable for final adjustments.' },
